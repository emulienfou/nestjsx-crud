...
export class Migration20231213224232 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "devices" ("device_key" uuid not null default uuid_generate_v4(), "description" text null, constraint "devices_pkey" primary key ("device_key"));');
    ...
    this.addSql('create table "projects" ("id" serial primary key, "created_at" timestamptz(0) null default CURRENT_TIMESTAMP, "updated_at" timestamptz(0) null default CURRENT_TIMESTAMP, "name" varchar(100) not null, "description" text null, "is_active" boolean not null default true, "company_id" int null);');
    ...
    this.addSql('create table "users" ("id" serial primary key, "created_at" timestamptz(0) null default CURRENT_TIMESTAMP, "updated_at" timestamptz(0) null default CURRENT_TIMESTAMP, "email" varchar(255) not null, "is_active" boolean not null default true, "name" varchar(255) not null, "deleted_at" timestamptz(0) null, "profile_id" int null, "company_id" int null);');
    this.addSql('alter table "users" add constraint "users_email_unique" unique ("email");');
    this.addSql('alter table "users" add constraint "users_profile_id_unique" unique ("profile_id");');

    this.addSql('create table "user_licenses" ("id" serial primary key, "user_id" int null, "license_id" int null, "years_active" int not null);');

    this.addSql('create table "user_projects" ("id" serial primary key, "review" varchar(255) null, "project_id" int null, "user_id" int null);');
    ...
    this.addSql('alter table "projects" add constraint "projects_company_id_foreign" foreign key ("company_id") references "companies" ("id") on update cascade on delete set null;');
    ...
    this.addSql('alter table "users" add constraint "users_company_id_foreign" foreign key ("company_id") references "companies" ("id") on update cascade on delete set null;');

    this.addSql('alter table "user_licenses" add constraint "user_licenses_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade on delete set null;');
    this.addSql('alter table "user_licenses" add constraint "user_licenses_license_id_foreign" foreign key ("license_id") references "licenses" ("id") on update cascade on delete set null;');
    ...
    this.addSql('alter table "user_projects" add constraint "user_projects_user_id_foreign" foreign key ("user_id") references "users" ("id") on update cascade on delete set null;');
